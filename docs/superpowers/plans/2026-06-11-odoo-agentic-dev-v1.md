...
- `project.stripBranchPrefixes` (default `["feature","feat","bugfix","bug","hotfix","fix","chore","task"]`, exported as `DEFAULT_STRIP_BRANCH_PREFIXES`) replaces the built-in list, and `deriveDatabaseName` strips AT MOST ONE leading type segment — the bash `case feature/*|...)` matches once, so `feature/fix/x` derives `<prefix>_fix_x`, not `<prefix>_x`.
- Context env assembly order is canonical → companion vars → aliases: each companion's `portEnv` lands in `ctx.env` as the port string and the new `urlEnv` as `http://localhost:<port>`; `envAliases` is free-form `Record<string, string>` whose targets are validated against the assembled env at `buildWorktreeContext` time (unknown target → `ConfigValidationError` naming the available keys). `CanonicalEnvVar` stays exported for compatibility only.
- `ComposeRef` carries `env: Record<string, string>` (populated from `ctx.env` by `prepareComposeFile`); every compose subprocess — run/tryRun/stream/waitForDb in `docker-compose.ts` plus the interactive shell/psql passthrough — passes it so project-supplied compose files interpolating `${ODOO_DATABASE:?}` / `${ODOO_HTTP_PORT:?}` work (the runner merges it over the parent env via `extendEnv: true`).



**Round-4 facts (added 2026-06-12, bash-glue absorption + publish prep, one commit per task):**
- Publishing: version `0.1.0-beta.1`, `publishConfig { access: "public", tag: "beta" }`, `prepublishOnly` runs the lint/typecheck/build/test gate; `pnpm pack` tarball verified to contain only `dist/` + `package.json` + `README.md` (the `files: ["dist"]` whitelist needed no change; `dist/testing/fake-adapters.*` ships as part of dist — deliberate, consumers use the fakes in their own tests). `Command.runWith` version string bumped to match. Publishing itself NOT done (auth pending).
- `run` command (`src/commands/run.ts`): env layering is parent env < ctx.env < `--env-file` files (later files win) — env files are EXPLICIT overrides, documented as such; this deliberately diverges from the legacy with-worktree-env.sh source-first ordering for predictability. dotenv subset: `KEY=value`, blank/`#` lines skipped, one layer of surrounding quotes stripped, no escapes/expansion/`export`. Missing env file → `ConfigLoadError` BEFORE spawning. Repeated flags in the v4 cli are `Flag.atLeast(0)` (there is no `Flag.repeated`); required variadic argv is `Argument.variadic({ min: 1 })`.
- `compose` command (`src/commands/compose.ts`): pure delegation to `runInteractivePassthrough` (same preamble/exit-code/recordEnvironment semantics as shell/psql); trailing args land verbatim after the canonical preamble.
- Two-level subcommand nesting WORKS in effect v4 beta cli: `worktree` is `Command.make("worktree")` + `Command.withSubcommands([create, remove])` nested under the root's own `withSubcommands` — help, parsing, and flag handling all behave; no flattening needed.
- `worktree create` hook contract: stdout purity is enforced by swapping `process.stdout.write` to stderr's writer for the duration (`withStdoutRedirectedToStderr`, the `exec 1>&2` trick) — covers Console.log AND runInherited child streaming since both resolve the property at call time; the single path line is written after restore. Setup failure in hook mode force-removes the half-made worktree (`git worktree remove --force`, best effort) before the error propagates; non-hook mode leaves it for inspection. Full stdout-purity e2e lives in the consuming repo (unit tests cover the pure pieces + orchestration via fakes).
- `worktree remove` hook contract: `runWorktreeRemoveHook` has error channel `never` (catch + catchDefect → log), so the process always exits 0; the shared-database guard applies on BOTH paths (dir-exists and dir-gone) and is checked before any docker call. Dir-gone fallback rebuilds the identity via `ODOO_WORKTREE_NAME=basename(path)` against the current root config and tears down with the existing `removeByLabel`.
- Recipe `worktree` section: `{ copyFiles?: string[]; branchPrefix?: string }` → normalized `{ copyFiles: [], branchPrefix: "worktree-" }`; NOT part of `computeTemplateKey` (template snapshots unaffected).
- `link-source` discovery: candidates are `<root>/../odoo` then `<dirname(wt)>/odoo` per `git worktree list --porcelain` path (deduped, first valid wins); validity = `odoo-bin` file + `odoo/` + `addons/` dirs (the legacy check). Explicit `--target`/`odoo.source` still skip validation entirely; `performLinkSource` now requires `CommandRunnerApi`.
- **v4 beta cli `--` bug discovered live (beta.78):** the lexer splits argv at the first `--` and attaches the trailing operands to the ROOT command's parsed arguments; the subcommand recursion gets `trailingOperands: []`, so post-`--` values NEVER reach a leaf's variadic Argument (and are silently dropped — the documented `psql -- -c ...` pattern was silently broken too; unit tests of the argv builders couldn't catch it). Fix: `src/commands/trailing-args.ts` recovers `process.argv` after the first `--` and run/compose/psql handlers concatenate it after the parsed variadic values; `run`/`compose` use plain `Argument.variadic()` (a `{ min: 1 }` would reject `run -- env` at parse time) and validate the combined argv in the handler. e2e regression: `run -- node -e ...` through the built CLI.
